...
// Helper function to check if command matches any alias
function commandMatches(command, commandName, aliases) {
    return command === commandName || (aliases && aliases.includes(command));
}
...
export default async function bettingHandler(m, sock, config) {
  try {
    ...
    // FIXED: Proper command matching logic with aliases
    const commandInfo = info.commands.find(c => 
        c.name === command || (c.aliases && c.aliases.includes(command))
    );
    ...
    } else if (commandMatches(command, 'fixtures', ['matches', 'games'])) {
        await handleFixtures(context, args.slice(1));
    } else if (commandMatches(command, 'betslip', ['slip'])) {
        await handleBetSlip(context, args.slice(1));
    } else if (commandMatches(command, 'mybets', ['bets'])) {
        await handleMyBets(context);
    } else if (commandMatches(command, 'bethistory', ['betlog'])) {
        await handleBetHistory(context);
    } else if (commandMatches(command, 'leagues', ['competitions'])) {
        await handleLeagues(context);
    } else if (commandMatches(command, 'results', ['recent', 'scores'])) {
...
async function handleLoadBetSlip(context, args) {
    ...
    const placedBet = await db.collection(BET_COLLECTIONS.BETS).findOne({ _id: { $regex: `${shareCode}// plugins/Football_betting.js - REFACTORED VERSION with Dynamic Form, UI Update, Placed Bet Sharing & Command Aliases
...
async function handleSharePlacedBet(context, args) {
    ...
    const placedBet = await db.collection(BET_COLLECTIONS.BETS).findOne({ 
        userId: senderId, 
        _id: { $regex: `${betId}// plugins/Football_betting.js - REFACTORED VERSION with Dynamic Form, UI Update, Placed Bet Sharing & Command Aliases
