...
// MIGRATED TO USE SHARED MongoDB CONNECTIONS
...
// ===== SCHEDULED TASK HANDLERS (Using shared connections) =====

async function processEquipmentBreakdown() {
  try {
    // Use PluginHelpers.safeDBOperation instead of creating new connection
    ...
    await safeOperation(async (db) => {
      ...
      for (const club of activeClubs) {
        ...
        if (baseRevenue > 0) {
          ...
          // Use unifiedUserManager for user balance
...
// ===== COMMAND HANDLERS (All using shared connections) =====
...
async function handleClubRegister(m, sock, args, userId) {
  ...
  try {
    ...
    // Use unifiedUserManager for money operations
...
async function handleClubBuy(m, sock, args, userId) {
  ...
  try {
    ...
    if (!equipment) {
      await sock.sendMessage(m.from, {
        text: `❌ Item "${itemName.replace(/_/g, ' ')}" not found!\n\nView available: /club market`
      });
      return;
    }
    ...
    if (club.equipment?.length >= 10) {
      await sock.sendMessage(m.from, {
        text: '❌ Maximum equipment limit reached (10 items)! Repair or sell existing ones.'
      });
      return;
    }
    ...
    // Use unifiedUserManager for money operations
    const moneyRemoved = await unifiedUserManager.removeMoney(userId, equipment.price, `Club equipment: ${itemName}`);
    
    if (!moneyRemoved) {
      await sock.sendMessage(m.from, {
        text: '❌ Failed to deduct payment. Please try again.'
      });
      return;
    }
...
💡 *Tip:* Hire a technician to reduce equipment wear!`;
    ...
    await sock.sendMessage(m.from, {
      text: '❌ Failed to purchase equipment. Please try again.'
    });
  }
}
...
// (The rest of the handlers follow the same pattern - using getCollection() and unifiedUserManager)
...
async function handleClubBook(m, sock, args, userId) {
  if (args.length < 2) {
    await sock.sendMessage(m.from, {
      text: '❌ Please specify celebrity and event!\n\n*Usage:* /club book <celebrity> <event>\n\nView available: /club market'
    });
    return;
  }
