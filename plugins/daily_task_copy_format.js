...
// NEW: Updated answer validation for copy-paste format with "Answer:" pattern
function validateAnswerFormat(text) {
  const answers = [];
  
  // Look for "Answer:" followed by actual answer text
  ...
  while ((match = answerPattern.exec(text)) !== null) {
    ...
    if (answer.length > 0 && answer.toLowerCase() !== 'answer:') {
      answers.push(answer);
    }
  }
  
  // Also support the old numbered format as fallback
  if (answers.length === 0) {
...
// UPDATED: New task message format with "Answer:" instead of "Type your answer here"
...
// UPDATED: Test function now supports both formats
async function handleTestTask(context, args) {
  const { reply } = context;
  const testAnswers = args.join(' ');
  
  if (!testAnswers) {
    await reply(`🔍 *Answer Format Test*\n\n*Usage:* ${context.config.PREFIX}testtask [test_message]\n\n*NEW FORMAT (Copy & Fill):*\n${context.config.PREFIX}testtask 1️⃣ Question 1?\n*Answer:* Asia\n\n2️⃣ Question 2?\n*Answer:* Abuja\n\n*OLD FORMAT (Still works):*\n${context.config.PREFIX}testtask 1. Asia 2. Abuja 3. 168\n\nTests your format without submitting.`);
    return;
  }
  ...
  try {
    ...
    result += `📝 Input length: ${testAnswers.length} characters\n\n`;
    ...
    if (answers.length >= taskSettings.questionCount) {
      ...
      result += `💡 *Copy the task message and fill in answers after each "Answer:" line*`;
    }
