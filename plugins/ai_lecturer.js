...
// FIX: Patched 'generateLecture' function to use POST for the primary API.
...
*
* --- PATCHED (Nov 7, 2025) ---
* Changed Primary API call from axios.get() to axios.post()
* to prevent "400 Bad Request" errors caused by overly long
* prompts being put into the URL.
* -------------------------------
*/
...
// === PROBLEM WAS HERE ===
// Using axios.get() put the entire 'combinedPrompt' into the URL,
// which caused a "400 Bad Request" because the URL was too long.
/*
const response = await axios.get(primaryApiUrl, {
params: { text: combinedPrompt }, // <--- This was the issue
timeout: CONFIG.PRIMARY_API_TIMEOUT
});
*/

// === THE FIX ===
// Use axios.post() to send the prompt in the request body,
// just like you correctly do for the Groq fallback.
const response = await axios.post(
primaryApiUrl,
{
// NOTE: You may need to confirm what body format this API expects.
// I am guessing it's { "text": ... } based on your old 'params'.
// It might also be { "prompt": ... } or something else.
"text": combinedPrompt
},
{
...
_ if (!lectureContent) throw new Error('AI returned empty script');
...
const date = new Date(entry.deliveredAt).toLocaleString('en-GB', {s
...
case 'lecture-log':
_     await handleLectureHistory(context);
break;
}
},
