// plugins/birthday_plugin.js - V3 (Converted for new plugin system)
...
export default {
  name: 'birthday',
  description: 'Advanced birthday system with automatic reminders and wishes',
  aliases: ['bday', 'birthdays', 'mybirthday', 'mybday'],
  category: 'social',
  usage: '[subcommand] [args]',
  example: 'birthday today | birthday upcoming 7 | mybirthday',
  version: '3.0.0',
  ...
  async run({ msg, args, text, command, sock, db, config, logger }) {
    try {
      ...
      const sender = msg.sender || msg.key.remoteJid;
      ...
      if (cmd === 'mybirthday' || cmd === 'mybday') {
        await handleMyBirthday({ msg, sock, config, sender, from, reply, db });
        return;
      }
      
      // Handle birthday main command
      if (args.length === 0) {
      ...
      await handleSubCommand(subCommand, subArgs, {
        msg, sock, config, sender, from, reply, db, logger
      });
      
    } catch (error) {
      logger.error(error, '❌ Birthday plugin error');
...
// Get all birthdays
async function getAllBirthdays(db) {
  try {
...
async function getBirthdayData(db, userId) {
  try {
    const collection = db.collection(COLLECTIONS.BIRTHDAYS);
...
async function getTodaysBirthdays(db) {
...
async function getUpcomingBirthdays(db, daysAhead) {
...
async function scheduledBirthdayReminders(context, daysAhead) {
  ...
  try {
    const upcomingBirthdays = await getUpcomingBirthdays(db, daysAhead);
...
async function scheduledCleanup(context) {
  ...
  try {
    ...
    logger.info(`🗑️ Cleaned up ${wishesResult.deletedCount} wishes and ${remindersResult.deletedCount} reminders`);
  } catch (error) {
...
async function showBirthdayMenu(reply, prefix) {
  const menuText = `🎂 *BIRTHDAY SYSTEM v3.0* 🎂\n\n` +
                  ...
                  `💡 *Quick:* ${prefix}mybirthday to view your birthday`;
...
async function handleToday(context) {
  const { sock, msg, db, config } = context;
...
async function handleUpcoming(context, args) {
  ...
  const allBirthdays = await getAllBirthdays(db);
...
async function handleAll(context) {
  const { sock, msg, sender, db, config } = context;
...
async function handleMyBirthday(context) {
  ...
  try {
    const birthdayData = await getBirthdayData(db, sender);
...
async function handleSettings(context, args) {
  const { reply, sender, db, config } = context;
...
async function handleForceWishes(context, args) {
  const { reply, sender, config } = context;
  ...
  switch (forceType) {
    case 'wishes':
      ...
      await scheduledBirthdayWishes(context);
    ...
    case 'reminders':
      ...
      await scheduledBirthdayReminders(context, days);
      await reply(`✅ Forced ${days}-day reminders completed`);
      break;
    case 'cleanup':
      await reply('🔧 Forcing cleanup...');
      await scheduledCleanup(context);
...
async function handleStatus(context) {
  ...
  try {
    const todaysBirthdays = await getTodaysBirthdays(db);
    const upcoming7 = await getUpcomingBirthdays(db, 7);
    const upcoming3 = await getUpcomingBirthdays(db, 3);
    const upcoming1 = await getUpcomingBirthdays(db, 1);
...
async function handleTest(context, args) {
  ...
  switch (testType) {
    ...
    case 'reminder':
      ...
      break;
    case 'today':
      await testTodayBirthdays(reply, context);
      break;
    case 'cleanup':
      await testCleanup(reply, context);
      break;
    default:
...
async function testTodayBirthdays(reply, context) {
...
async function testCleanup(reply, context) {
...
async function handleGroups(context, args) {
  const { reply, sender, msg, config } = context;
  ...
  switch (action) {
    case 'add':
      await addGroup(reply, msg, context);
      break;
    case 'remove':
