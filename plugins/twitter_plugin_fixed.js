...
// FIXED: Get user ID by username with proper error handling
...
// FIXED: Get user tweets with corrected parameters
async function getUserTweets(userId, sinceId = null) {
  try {
    ...
    // FIXED: Only add expansions and media fields if we're including media
    ...
    // FIXED: Handle exclude parameters properly
    const excludeParams = [];
      ...
      case 'maxtweets':
        if (!value || isNaN(value)) {
          ...
          if (count < 1 || count > 100) {
            responseText = `⚠️ Max tweets must be between 1 and 100.`;
          } else {
