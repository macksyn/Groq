...
const MAX_CRASHES_BEFORE_DISABLE = 5;
const PLUGIN_EXECUTION_TIMEOUT_MS = 480000;
...
class PluginManager {
  ...
  setReferences(sock, config, bot) {
    ...
    // Register group participant event forwarder so plugins with
    // `groupEventHandlers` can react to participant add/remove events.
    try {
      if (sock && sock.ev && !this._registeredGroupListeners) {
        sock.ev.on('group-participants.update', async (update) => {
          try {
            // Baileys may emit either a single update object or an array of updates
            const updates = Array.isArray(update) ? update : [update];

            for (const single of updates) {
              // single: { id, participants, action }
              for (const [filename, plugin] of this.plugins.entries()) {
                ...
                const handlers = plugin.info?.groupEventHandlers;
                if (!handlers) continue;

                const key = `participants.${single.action}`;
                const handler = handlers[key];
                if (typeof handler === 'function') {
                  try {
                    await handler(sock, { id: single.id, participants: single.participants }, logger);
                  } catch (err) {
                    logger.error(err, `❌ Error in groupEventHandler for ${filename}`);
                  }
                }
              }
            }
          } catch (err) {
            logger.error(err, '❌ Error forwarding group-participants.update to plugins');
          }
        });
        this._registeredGroupListeners = true;
      }
    } catch (err) {
      logger.error(err, '❌ Failed to register plugin group event forwarder');
    }
  }
  ...
  async loadPlugin(filename) {
    ...
    try {
      ...
      const pluginInfo = {
        ...
        executeOnAllMessages: pluginData.executeOnAllMessages || false, // ADD THIS LINE
      };
  ...
  async executePlugins(m, sock, config, bot) {
    for (const [filename, plugin] of this.plugins.entries()) {
      ...
      // NEW FIX: Check if plugin should execute on all messages
      const hasCommands = (plugin.commands && plugin.commands.length > 0) || (plugin.aliases && plugin.aliases.length > 0);
      const executeOnAll = plugin.info?.executeOnAllMessages === true;

      // Skip command plugins UNLESS they have executeOnAllMessages flag
      if (hasCommands && !executeOnAll) {
        continue;
      }
