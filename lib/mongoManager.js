...
// Auto-connect on import in production - FIXED with better error handling
if (process.env.NODE_ENV === 'production' && process.env.MONGODB_URI) {
  // Delay auto-connect to allow proper initialization
  setTimeout(() => {
    mongoManager.connect().catch(error => {
      console.warn(chalk.yellow('⚠️ Auto-connect failed:', error.message));
      console.log(chalk.blue('💡 MongoDB will be initialized when the main app starts'));
    });
  }, 2000); // 2 second delay
}
