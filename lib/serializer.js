...
export function decodeJid(jid, sock = null, groupId = null) {
  ...
  try {
    ...
    // FIXED: Handle @lid format - convert to @s.whatsapp.net immediately
    if (jid.includes('@lid')) {
      ...
      if (/^\d+$/.test(numberPart)) {
        ...
        idMappingCache.set(jid, converted);
        return converted;
      }
...
async function resolveGroupMemberIds(sock, groupId, lidIds) {
  ...
  try {
    ...
    const resolvedIds = lidIds.map(lidId => {
      ...
      // Fallback: convert @lid to @s.whatsapp.net format
...
export function serializeMessage(m, sock) {
  ...
  // FIXED: Ensure sender is ALWAYS in @s.whatsapp.net format
  if (m.isGroup) {
    const rawSender = m.key?.participant;
    m.sender = decodeJid(rawSender, sock, m.from) || '';

    // CRITICAL: Validate and normalize the sender
    if (m.sender) {
      const normalized = validateAndNormalizeJid(m.sender);
      if (normalized) {
        m.sender = normalized;
      } else {
        console.warn(`⚠️ Could not normalize sender in group: ${rawSender}`);
        // Fallback: try basic conversion
        if (rawSender && rawSender.includes('@lid')) {
          const numberPart = rawSender.replace('@lid', '').split(':')[0];
          if (/^\d+$/.test(numberPart)) {
            m.sender = numberPart + '@s.whatsapp.net';
          } else {
            m.sender = '';
          }
        } else {
    ...
    try {
      ...
      try {
        ...
        if (quoted?.quotedMessage) {
          ...
          let quotedSender = decodeJid(quoted.participant, sock, m.from) || '';

          // Validate quoted sender
          if (quotedSender) {
            const normalized = validateAndNormalizeJid(quotedSender);
            if (normalized) {
              quotedSender = normalized;
            }
          }
      ...
      try {
        ...
        if (Array.isArray(rawMentions) && rawMentions.length > 0) {
          ...
          m.mentions = rawMentions.map(mention => {
            ...
            const normalized = validateAndNormalizeJid(decoded);
