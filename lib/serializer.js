  ...
  try {
    ...
    const resolvedIds = lidIds.map(lidId => {
      ...
      const participant = metadata.participants.find(p => {
        const lidNumber = lidId.replace('@lid', '').split(':')[0];
        // Match against the LID part or the full LID
      ...
      // Fallback: convert @lid to @s.whatsapp.net format (the broken way)
      const numberPart = lidId.replace('@lid', '').split(':')[0];
      if (/^\d+$/.test(numberPart)) {
        const converted = numberPart + '@s.whatsapp.net';
        // idMappingCache.set(lidId, converted); // Do not cache this
        return converted;
      }
...
export async function serializeMessage(m, sock) {
  ...
  if (m.isGroup) {
    const rawSender = m.key?.participant; // This is the @lid JID (e.g., 12345@lid:10)

    // Check cache first
    let resolvedSender = idMappingCache.get(rawSender);

    if (!resolvedSender && rawSender && rawSender.includes('@lid')) {
      // It's a LID and not in cache, we MUST resolve it by fetching group metadata
      try {
        const metadata = await sock.groupMetadata(m.from); // m.from is group JID
        const participant = metadata.participants.find(p => 
          p.id.includes(rawSender.split(':')[0]) || p.id === rawSender
        );

        if (participant && participant.id.includes('@s.whatsapp.net')) {
          resolvedSender = participant.id; // FOUND IT! e.g., 23480...@s.whatsapp.net
          idMappingCache.set(rawSender, resolvedSender); // Cache the correct mapping
        } else {
          // Could not resolve, fallback to broken method
          resolvedSender = decodeJid(rawSender, sock, m.from);
          console.warn(`⚠️ Could not resolve LID ${rawSender} to a real JID.`);
        }
      } catch (e) {
        console.error(`🔴 CRITICAL: Failed to fetch metadata to resolve LID ${rawSender}: ${e.message}`);
        resolvedSender = decodeJid(rawSender, sock, m.from); // Fallback
      }
    } else if (!resolvedSender) {
      // Not a LID or not in cache, just use regular decode
      resolvedSender = decodeJid(rawSender, sock, m.from);
    }

    m.sender = resolvedSender || ''; // Assign the resolved sender
